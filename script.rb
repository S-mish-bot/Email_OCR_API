    ...
    SERVICE.list_user_labels(user_id).labels.each do |label|
      if label.name == 'UNREAD' then
        ...
        emails_received.messages.each do |i|
          ...
          attachment_id = email.payload.parts.find{|p| p.mime_type == 'application/pdf'}.body.attachment_id
          email = SERVICE.get_user_message_attachment(user_id, msg_id, attachment_id)
          file_name = "Invoice_#{Time.now.strftime('%Y%m%d%H%M%S%12N')}.pdf"
          File.open(file_name, 'wb') {|f| f.puts (email.data)}
